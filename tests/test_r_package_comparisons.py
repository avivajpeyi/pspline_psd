...
def test_mcmc_comparison(helpers):
    ...
    plt.plot(psd_x, py_mcmc.psd_quantiles[0], color='tab:orange', alpha=0.5, label='Python (Uniform CI)')
    plt.fill_between(psd_x, py_mcmc.psd_quantiles[1], py_mcmc.psd_quantiles[2], color='tab:orange', alpha=0.2, linewidth=0.0)
    plt.plot(psd_x, r_mcmc.psd_quantiles[0], color='tab:green', alpha=0.5, label='R (Uniform CI)')
    plt.fill_between(psd_x, r_mcmc.psd_quantiles[1], r_mcmc.psd_quantiles[2], color='tab:green', alpha=0.2, linewidth=0.0)
    # turn off grid
    plt.grid(False)
